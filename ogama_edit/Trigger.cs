        ...
        public Trigger()
    {
      ...
      this.ipHost = Dns.GetHostEntry(Dns.GetHostName());
       ...
       this.localEndPoint = new IPEndPoint(ipAddr, 11111);
        this.sender= sender = new Socket(ipAddr.AddressFamily,SocketType.Stream, ProtocolType.Tcp);
        }
    ...
    public void Send()
    {
      switch (this.outputDevice)
      {
        ...
        case TriggerOutputDevices.LPT:
          ...
          break;
        case TriggerOutputDevices.TCP:
          this.SendTriggerToTCP();
                    break;
        ...
        private void SendTriggerToTCP()
        {
            sender.Connect(localEndPoint);
            byte[] messageSent = Encoding.ASCII.GetBytes(this.signalValue);
            this.sender.Send(messageSent);
            Thread.sleep(this.signalingTime);
        }
